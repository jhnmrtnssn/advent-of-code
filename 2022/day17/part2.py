...
class Rock:
    def __init__(self, rtype):
        ...
        self.spawn()
        self.parts = self.get_block_parts()
...
class Tetris:
    ...
    def spawn_block(self):
        new_rock = Rock(self.block_type)
        new_block = new_rock.block
        self.block_parts = new_rock.parts
    ...
    def play_number_of_rounds(self, n_rounds):
        ...
        while played_rounds < n_rounds:
            if self.turn % len(self.moves) == 3:  # Insane luck to find this...
                new_score = self.get_tower_height()
                if new_score - prev_score == diff_score:
                    if found_repeating:
                        diff_rounds = played_rounds - first_repeating_round
                        repeating_score, played_rounds = self.calc_remaining_points(
                            played_rounds, diff_rounds, diff_score
                        )
                        self.repeating_score = repeating_score
                    first_repeating_round = played_rounds
                    found_repeating = True
                diff_score = new_score - prev_score
    ...
    def calc_remaining_points(self, played_rounds, diff_rounds, diff_score):
        max_rounds = 1000000000000
        total_remaining_rounds = max_rounds - played_rounds
        repeating_score = math.floor(total_remaining_rounds / diff_rounds) * diff_score
        remaining_rounds = max_rounds - (total_remaining_rounds % diff_rounds)
        return repeating_score, remaining_rounds

    def get_tower_height(self):
        return len(self.map) - 1 + self.repeating_score


def main():
    data = parse_input("input.txt")
    tetris = Tetris(data)
    tetris.play_number_of_rounds(1000000000000)
