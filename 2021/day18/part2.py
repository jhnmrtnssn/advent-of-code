...
class SnailfishAdder:
    ...
    def iterate_sum_for_all_values(self):
        for i, left_sum in enumerate(self.numbers):
            for j, right_sum in enumerate(self.numbers):
                if i != j:
                    self.sum = []
                    self.sum = [deepcopy(left_sum), deepcopy(right_sum)]
                    self.calc_sum()
                    magnitude = calc_magnitude(self.sum)
...
sf_adder.iterate_sum_for_all_values()

#print("Part 2:", calc_magnitude(total_sum))
