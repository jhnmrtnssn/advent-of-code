...
class OreCollector:
    ...
    def recursive_robot_buyer(self, minutes_left):
        ...
        state = (
            tuple(
                [
                    self.income[0] + self.new_income[0],
                    self.income[1] + self.new_income[1],
                    self.income[2] + self.new_income[2],
                    self.income[3] + self.new_income[3],
                ]
            ),
        ...
        for robot in get_tactics(sum(curr_income) + sum(curr_new_income)):
            ...
            if minutes_left < 8 and self.income[GEODE] == 0:
                break
...
def calc_quality_level(data):
    ...
    for key in data.keys():
        ...
        ore_collector.recursive_robot_buyer(minutes_left=32)
        geodes.append(ore_collector.best_score)
        print(id_number)
        print(geodes[id_number - 1])
        print("-----------")
        if id_number == 3:
            break
...
def main():
    ...
    calc_quality_level(data)  # 38220 too low
