...
def calc_node_risk(acc_risk_map, prev_val, x, y):
    new_val = full_risk_map[x][y] + prev_val
    if acc_risk_map[x][y] > new_val:
        acc_risk_map[x][y] = new_val
    return acc_risk_map


def create_full_map(risk_map):
    dim_x = risk_map.shape[0]
    dim_y = risk_map.shape[1]
    full_map = np.zeros([dim_x * 5, dim_y * 5])
    for x_incr in range(0, 5):
        for y_incr in range(0, 5):
            temp_risk_map = deepcopy(risk_map)
            for x, row in enumerate(temp_risk_map):
                for y, value in enumerate(row):
                    if value + x_incr + y_incr > 9:
                        temp_risk_map[x][y] = value + x_incr + y_incr - 9
                    else:
                        temp_risk_map[x][y] += x_incr + y_incr
            full_map[dim_x * x_incr : dim_x * (x_incr + 1),
                     dim_y * y_incr : dim_y * (y_incr + 1)] = temp_risk_map
...
acc_risk_map = np.ones(full_risk_map.shape) * np.inf
full_risk_map[0][0] = 0
start_node = [[0, 0]]
...
while len(current_nodes) > 0 or len(rescan_list) > 0:
    ...
    for rescan_node in rescan_list:
        ...
        prev_val = find_lowest_prev_val(acc_risk_map, x, y, rescan)
        if prev_val + full_risk_map[x][y] < node_val:
            acc_risk_map[x][y] = prev_val + full_risk_map[x][y]
            current_nodes.extend(add_rescan_neighbors(full_risk_map, x, y))
