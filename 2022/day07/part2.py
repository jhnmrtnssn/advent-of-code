...
class FileSystem:
    ...
    def find_directory_to_delete(self):
        file_system_size = self.folders[str(["/"])]
        min_deleted_size = 30000000 - (70000000 - file_system_size)

        found_deleted_size = file_system_size
        for value in self.folders.values():
            if min_deleted_size < value < found_deleted_size:
                found_deleted_size = value

        return found_deleted_size
...
def main():
    ...
    print(file_system.find_directory_to_delete())
