...
class PressureOptimizer:
    def __init__(self, data: Dict):
        ...
        self.start = self.valves["AA"]
        self.traveled: List[Dict] = []
        self.time_left = 30
        self.best_score = 0
        print(self.simulate_best_score(self.start, 0, [], self.time_left))
    ...
    def travel_and_open_valve(self, start_valve, goal_valve, time_left):
        dist = self.get_shortest_distance(start_valve, goal_valve) + 1
        if time_left - dist < 0:
            return time_left, 0
        time_left -= dist
        return time_left, time_left * goal_valve["flow"]
    ...
    def simulate_best_score(self, start_valve, score, traveled, time_left):
        best_score = 0
        prev_score = score
        for valve_name in self.flow_valves:
            ...
            if valve_name in traveled:
                continue
            new_time_left, travel_score = self.travel_and_open_valve(
                start_valve, self.valves[valve_name], time_left
            )
            if travel_score > 0:
                traveled.append(valve_name)
                new_score += self.simulate_best_score(
                    self.valves[valve_name], travel_score, traveled, new_time_left
                )
